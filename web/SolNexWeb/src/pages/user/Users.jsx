      ...
      try {
        ...
        if (isCurrent) {
          setError(loadError.status === 401
            ? "Backoffice sign-in is required to view user management. Please sign in again."
            : loadError.message)
        }
      } finally {
      ...
      <div className="overflow-x-auto rounded-lg border bg-background">
        <table className="w-full min-w-[780px] text-left text-sm">
          <thead className="border-b bg-muted/50 text-xs uppercase tracking-wide text-muted-foreground">
            <tr><th className="px-4 py-3">User</th><th className="px-4 py-3">NIC</th><th className="px-4 py-3">Role</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Actions</th></tr>
          </thead>
          <tbody className="divide-y">
            {isLoading && <tr><td colSpan="5" className="px-4 py-10 text-center text-muted-foreground">Loading users...</td></tr>}
            {!isLoading && users.length === 0 && <tr><td colSpan="5" className="px-4 py-10 text-center text-muted-foreground">No users found.</td></tr>}
            {!isLoading && users.map((user) => (
              <tr key={user.nic} className="hover:bg-muted/30">
                ...
                <td className="px-4 py-3"><select value={user.role} onChange={(event) => changeRole(user, event.target.value)} disabled={workingNic === user.nic} className="h-9 rounded-md border bg-background px-2 text-sm"><option value="Backoffice">Backoffice</option><option value="GridOperator">Grid Operator</option><option value="Prosumer">Prosumer</option></select></td>
                ...
                <td className="px-4 py-3 text-right"><button type="button" title={user.accountStatus === "Active" ? "Deactivate user" : "Activate user"} onClick={() => changeStatus(user)} disabled={workingNic === user.nic} className="inline-flex size-9 items-center justify-center rounded-md border hover:bg-muted disabled:opacity-50">{user.accountStatus === "Active" ? <UserRoundX className="size-4" /> : <UserCheck className="size-4" />}</button></td>
              </tr>
            ))}
          </tbody>
        </table>
